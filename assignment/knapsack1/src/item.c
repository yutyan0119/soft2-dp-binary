...
Item get_item(const Itemset *list, size_t index) { return list->item[index]; }

double get_itemweight(Item item) { return item.weight; }

double get_itemvalue(Item item) { return item.value; }
...
Itemset *load_itemset(char *filename) {
  ...
  double value[num];
  double weight[num];
  fread(value, sizeof(double), num, fp);
  fread(weight, sizeof(double), num, fp);
  Itemset *list = (Itemset *)malloc(sizeof(Itemset));
  ...
  *list = (Itemset){.number = num, .item = item};
  fclose(fp);
  return list;
}
