...
double solve(const Itemset *list, double capacity) {
  ...
  // !!最大の組み合わせが返ってくる訳ではない!!
  unsigned char *flags =
      (unsigned char *)calloc(list->number, sizeof(unsigned char));
...
// 再帰的な探索関数
double search(int index, const Itemset *list, double capacity,
  ...
  const double v1 =
      search(index + 1, list, capacity, flags, sum_v + get_itemvalue(item),
